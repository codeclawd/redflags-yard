...
GUARD_CMD=""          # optional: must exit 0 after a good benchmark, else the round is discarded
...
_cmd_pid=$!
# The watcher's `sleep` gets a unique argv[0] so it can be killed by name after the watcher
# is dead — an orphaned sleep keeps stdout open and hangs any pipe reading this script.
_tag="autoresearch-watch-$$"
( bash -c "exec -a '$_tag' sleep '$TIMEOUT_SEC'"; kill -9 "$_cmd_pid" 2>/dev/null ) &
_watch_pid=$!
wait "$_cmd_pid" 2>/dev/null
_rc=$?
kill -9 "$_watch_pid" 2>/dev/null
wait "$_watch_pid" 2>/dev/null
pkill -9 -f "^$_tag " 2>/dev/null
_end=$(date +%s)
...
if [ "$_status" = "ok" ] && [ -z "$_metric" ]; then _status="crash"; fi
if [ -n "$_correct" ] && [ "$_correct" != "true" ]; then _status="incorrect"; fi

# ---- guard: only worth running when the benchmark itself passed -------------
_guard="skipped"
if [ "$_status" = "ok" ] && [ -n "$GUARD_CMD" ]; then
  if bash -c "$GUARD_CMD" > "$BENCH_LOG.guard" 2>&1; then
    _guard="pass"
  else
    _guard="fail"; _status="guard_failed"
    tail -n 30 "$BENCH_LOG.guard"
  fi
  rm -f "$BENCH_LOG.guard"
fi
