...
const ROW =
  "grid grid-cols-[minmax(96px,1fr)_64px_repeat(8,minmax(0,84px))] items-center lg:grid-cols-[minmax(132px,1fr)_84px_repeat(8,minmax(0,100px))]";
  ...
  return (
    <section aria-labelledby="compare-heading" className="flex flex-col gap-3">
      ...
      <div className="-mx-1 overflow-x-auto px-1">
        <div className="w-full min-w-[860px]">
          <div className={`${ROW} border-b border-rope pb-2`}>
            <span className="pl-2 text-[13px] text-amber-dim">App</span>
            <span className="text-[13px] text-amber-dim">Score</span>
              ...
              return (
                <li key={ship.id}>
                  <button
                    ...
                    aria-label={`Scan ${ship.name} — score ${ship.score.value} of 100, grade ${ship.score.grade}, ${ship.flagCount} charges, ${found} of ${matrix.columns.length} columns found`}
                    className={`${ROW} w-full py-2 text-left transition-colors disabled:opacity-60 ${
                      active ? "bg-ink-2 shadow-[inset_2px_0_0_var(--color-amber)]" : "hover:bg-ink-2"
                    }`}
                  >
                    <span aria-hidden className="truncate pl-2 text-[15px] text-parchment">
                      {ship.name}
                    </span>
      ...
      <p className="max-w-[80ch] text-[13px] leading-snug text-amber-dim">
        ...
        Scanned {scanned} · rules only, AI check off.
      </p>
    </section>
  );
}
